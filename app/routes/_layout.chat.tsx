...
import {  type ClientLoaderFunctionArgs, useLoaderData, json}from "@remix-run/react";

import { chat, getCurrentModel } from "~/llmapi/llama";
...
function CommandCopy({txt,btnTxt="Copy", color="black"}) {
const [datatip,setDataTip]=useState(btnTxt);
  ...
  return (
    <div className='tooltip tooltip-info' data-tip={datatip}>
...
export default function MyComponent() {
  ...
  if (done) {
    // save this prompt and result for future use
  ...
  return (
    <div className='pt-12  text-2xl text-wrap'> 
        ...
        <div className="flex flex-row items-center text-sm font-thin text-white overflow-hidden"> 
            <CommandCopy txt={result} btnTxt="Copy Response" color="white"></CommandCopy> 
            <CommandCopy txt={prompt} color="white" btnTxt="Copy Prompt"></CommandCopy> 
            <CommandCopy txt={prompt+"\n\n"+result} btnTxt="Copy Both" color="white"></CommandCopy> 
             {statStr} <pre>{"    |  NOTE: "}{ getCurrentModel()+" can make mistakes. Check before using it" }</pre> 
             </div>
            </div>:""}
          <Prompt></Prompt>
    </div>
